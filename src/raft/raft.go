...
func (rf *Raft) sendInstallSnapshot(server int, request *InstallSnapshotArgs, response *InstallSnapshotReply) bool {
	utils.Debug(utils.DInfo, "S%d send InstallSnapshot request to %d {%+v}", rf.me, server, request)
	...
	utils.Debug(utils.DInfo, "S%d get InstallSnapshot response from %d {%+v}", rf.me, server, response)
	return ok
}
